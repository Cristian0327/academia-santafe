...
} from '\var\task\academia-santafe/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\academia-santafe/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\academia-santafe')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\academia-santafe',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\academia-santafe\.netlify\dist\run\handlers\server.js')
