...
export default function PerfilPage() {
  ...
  const [editImage, setEditImage] = useState(sessionData?.user?.image || '');
  ...
  useEffect(() => {
    if (sessionData) {
      ...
      setEditImage(sessionData.user?.image || '');
    }
  }, [sessionData]);
  ...
  const cargarDatosUsuario = async () => {
    ...
    setCargando(true);
    try {
      // Cargar inscripciones desde localStorage (sin DB)
      const stored = localStorage.getItem(`inscripciones_${sessionData.user.id}`);
      const inscripciones = stored ? JSON.parse(stored) : [];

      // Separar cursos completados y en progreso
      const completados = inscripciones?.filter((i: any) => i.completado) || [];
      const enProgreso = inscripciones?.filter((i: any) => !i.completado) || [];

      setCursosCompletados(completados.map((i: any) => ({
        id: i.curso_id,
      ...
      setCursosEnProgreso(enProgreso.map((i: any) => ({
        id: i.curso_id,
        titulo: i.cursos?.titulo || 'Sin título',
        categoria: i.cursos?.categoria || 'General',
        progreso: i.progreso,
      ...
      const certsStored = localStorage.getItem(`certificados_${sessionData.user.id}`);
      const certs = certsStored ? JSON.parse(certsStored) : [];
      setCertificados(certs || []);

      // Calcular estadísticas
      const totalHoras = inscripciones?.reduce((sum: number, i: any) => {
              ...
              <div className="flex flex-col md:flex-row items-center md:items-start gap-3 text-gray-600 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-sm font-semibold">
                    {sessionData.user?.role === 'admin' ? 'Administrador' : 
                     sessionData.user?.role === 'instructor' ? 'Instructor' : 'Estudiante'}
                  </span>
                </div>
              </div>
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    localStorage.setItem('user_profile', JSON.stringify({
                      ...
                      rol: sessionData.user.role || 'student',
                    ...
                    // 4. Actualizar la sesión de NextAuth
                    await update({
                      trigger: 'update',
                      user: {
                        name: editName,
