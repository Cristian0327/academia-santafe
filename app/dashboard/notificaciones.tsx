...
export default function NotificacionesPage() {
  ...
  const cargarNotificaciones = async () => {
    ...
    setCargando(true);
    try {
      // Cargar notificaciones desde localStorage (sin DB)
      const stored = localStorage.getItem(`notificaciones_${sessionData.user.id}`);
  ...
  const marcarComoLeida = async (id: string) => {
    try {
      const updated = notificaciones.map(n => n.id === id ? { ...n, leido: true } : n);
      setNotificaciones(updated);
      if (sessionData?.user?.id) {
        localStorage.setItem(`notificaciones_${sessionData.user.id}`, JSON.stringify(updated));
      }
    } catch (error) {
  ...
  const marcarTodasComoLeidas = async () => {
    try {
      const updated = notificaciones.map(n => ({ ...n, leido: true }));
      setNotificaciones(updated);
      if (sessionData?.user?.id) {
        localStorage.setItem(`notificaciones_${sessionData.user.id}`, JSON.stringify(updated));
      }
