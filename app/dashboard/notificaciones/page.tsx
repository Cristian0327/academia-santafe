...
export default function NotificacionesPage() {
  ...
  const cargarNotificaciones = async () => {
    ...
    try {
      const response = await fetch(`/api/notificaciones?userId=${sessionData.user.id}`);
      ...
      if (notifServidor && notifServidor.length > 0) {
        ...
        localStorage.setItem(`notificaciones_${sessionData.user.id}`, JSON.stringify(notifServidor));
        setCargando(false);
        return;
      }
    } catch (error) {
    ...
    localStorage.setItem(`notificaciones_${sessionData.user.id}`, JSON.stringify(notificacionesEjemplo));
    setCargando(false);
  };
  ...
  const marcarComoLeida = async (id: string) => {
    ...
    localStorage.setItem(`notificaciones_${sessionData?.user?.id}`, JSON.stringify(notificacionesActualizadas));
