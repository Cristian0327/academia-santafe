...
export default function DashboardPage() {
  ...
  const cargarDatosUsuario = async () => {
    ...
    try {
      ...
      const cursosActivos = inscripciones?.filter((i: any) => !i.completado).length || 0;
      const promedioProgreso = inscripciones && inscripciones.length > 0
        ? Math.round(inscripciones.reduce((sum: number, i: any) => sum + i.progreso, 0) / inscripciones.length)
        : 0;
      
      const horasEstimadas = inscripciones?.reduce((sum: number, i: any) => {
        const duracion = i.cursos?.duracion || '0';
        const horas = parseFloat(duracion.replace(/[^0-9.]/g, '')) || 0;
        return sum + (horas * (i.progreso / 100));
      }, 0) || 0;

      const certificadosCount = inscripciones?.filter((i: any) => i.completado).length || 0;
      ...
      if (response.ok) {
        ...
        const idsInscritos = inscripciones?.map((i: any) => i.curso_id) || [];
        const cursosNoInscritos = cursos.filter((c: any) => !idsInscritos.includes(c.id)).slice(0, 3);
        setTodosLosCursos(cursosNoInscritos);
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">
            ¡Hola, {sessionData?.user?.name || 'Usuario'}! 👋
          </h1>
