  ...
  callbacks: {
    async session({ session, token }: any) {
      if (session?.user) {
        ...
        // Cargar desde localStorage (sin DB)
        let datosUsuario = null;
        try {
          // En servidor, no hay localStorage, usar datos del token
          if (typeof window !== 'undefined') {
            const stored = localStorage.getItem('user_profile');
            if (stored) {
              datosUsuario = JSON.parse(stored);
            }
          }
        } catch (error) {
        ...
        if (datosUsuario) {
          session.user.name = datosUsuario.nombre || token.name;
          session.user.image = datosUsuario.imagen || token.picture;
          session.user.role = datosUsuario.rol || 'student';
        } else {
          // Si no, usar los datos del token
