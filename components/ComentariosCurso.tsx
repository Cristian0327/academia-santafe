...
export default function ComentariosCurso({ cursoId }: ComentariosCursoProps) {
  ...
  const eliminarComentario = async (id: number) => {
    ...
    try {
      const stored = localStorage.getItem(`comentarios_${cursoId}`);
      const comentariosActuales = stored ? JSON.parse(stored) : [];
      const comentariosFiltrados = comentariosActuales.filter((c: any) => 
        c.id !== id.toString() || c.usuario_id !== session?.user?.email
      );
      localStorage.setItem(`comentarios_${cursoId}`, JSON.stringify(comentariosFiltrados));
      cargarComentarios();
    } catch (error) {
  ...
  const guardarEdicion = async (id: number) => {
    ...
    try {
      const stored = localStorage.getItem(`comentarios_${cursoId}`);
      const comentariosActuales = stored ? JSON.parse(stored) : [];
      const comentariosActualizados = comentariosActuales.map((c: any) => {
        if (c.id === id.toString() && c.usuario_id === session?.user?.email) {
          return {
            ...c,
